...
goodname:  this.data.detailList.dingdanbianhao,
...
if (id) {
const {data} = await detail("yaopindingdan",id)
this.setData({
payAuth:utils.isAuthFront('yaopindingdan','支付')
})
...
this.setData({
title:  this.data.detailList.dingdanbianhao,
    });
...
const resUpdate = await update('yaopindingdan', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','yaopindingdan');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yaopindingdan", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','yaopindingdan');
