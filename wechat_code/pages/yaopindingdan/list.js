...
res = await page(`yaopindingdan`, searchForm);
} else {
res = await list(`yaopindingdan`, searchForm);
}
...
wx.navigateTo({
url: `/pages/yaopindingdan/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yaopindingdan",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/yaopindingdan/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/yaopindingdan/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 yaopindingdan Yaopindingdan yaopindingdan ${pathname}
console.log('Yaopindingdan  yaopindingdan')
isAuthObj['addAuth']   = utils.isAuth("yaopindingdan", "新增")
isAuthObj['delAuth']  = utils.isAuth("yaopindingdan", "删除")
isAuthObj['editAuth']   =utils.isAuth("yaopindingdan", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("yaopindingdan", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("yaopindingdan", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("yaopindingdan", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("yaopindingdan",obj)
    }else{
         resList= await list("yaopindingdan",obj)
    }
