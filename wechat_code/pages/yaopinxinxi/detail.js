...
rukujiluisAuthFront:'',
isAuthrukujilu:'',
chukujiluisAuthFront:'',
isAuthchukujilu:'',
},
async    onLoad(option) {
let authobj={}
authobj['pageIsAuth']= wx.getStorageSync('isAuth')
authobj['isAuthrukujilu']= utils.isAuth('yaopinxinxi','入库')
authobj['isAuthchukujilu']= utils.isAuth('yaopinxinxi','出库')
authobj['rukujiluisAuthFront']=  utils.isAuthFront('yaopinxinxi','入库')
authobj['chukujiluisAuthFront']=  utils.isAuthFront('yaopinxinxi','出库')
...
goodname:  this.data.detailList.yaopinmingcheng,
...
data['total'] = parseFloat(data['price'] * this.data.activeSeat.length).toFixed(2)
wx.showModal({
title: '提示',
content: '是否预订选中座位',
complete: async (res) => {
...
if (id) {
const {data} = await detail("yaopinxinxi",id)
this.setData({
payAuth:utils.isAuthFront('yaopinxinxi','支付')
})
...
let objClo={}
objClo['picture']= data.yaopintupian.split(',');
data.yaopinxiangqing=utils.addImgWidthStyle(data.yaopinxiangqing,'100%')
...
this.setData({
title:  this.data.detailList.yaopinmingcheng,
    });
...
const resUpdate = await update('yaopinxinxi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["yaopintupian"] = data.yaopintupian
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','yaopinxinxi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yaopinxinxi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','yaopinxinxi');
