...
kaoqinjiluisAuthFront:'',
isAuthkaoqinjilu:'',
},
async    onLoad(option) {
let authobj={}
authobj['pageIsAuth']= wx.getStorageSync('isAuth')
authobj['isAuthkaoqinjilu']= utils.isAuth('yuangong','签到')
authobj['kaoqinjiluisAuthFront']=  utils.isAuthFront('yuangong','签到')
...
goodname:  this.data.detailList.yuangongzhanghao,
...
if (id) {
const {data} = await detail("yuangong",id)
this.setData({
payAuth:utils.isAuthFront('yuangong','支付')
})
...
let objClo={}
objClo['picture']= data.touxiang.split(',');
...
this.setData({
title:  this.data.detailList.yuangongzhanghao,
    });
...
const resUpdate = await update('yuangong', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','yuangong');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yuangong", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','yuangong');
