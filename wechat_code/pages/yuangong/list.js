...
if(this.data.yuangongzhanghao){
    searchForm['yuangongzhanghao'] = '%' + this.data.yuangongzhanghao + '%'
}
...
res = await page(`yuangong`, searchForm);
} else {
res = await list(`yuangong`, searchForm);
}
...
let targetName="yuangongzhanghao"
...
wx.navigateTo({
url: `/pages/yuangong/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yuangong",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/yuangong/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/yuangong/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 yuangong Yuangong yuangong ${pathname}
console.log('Yuangong  yuangong')
isAuthObj['addAuth']   = utils.isAuth("yuangong", "新增")
isAuthObj['delAuth']  = utils.isAuth("yuangong", "删除")
isAuthObj['editAuth']   =utils.isAuth("yuangong", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("yuangong", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("yuangong", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("yuangong", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("yuangong",obj)
    }else{
         resList= await list("yuangong",obj)
    }
