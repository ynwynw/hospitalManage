...
if(this.data.yaopinleixing){
    searchForm['yaopinleixing'] = '%' + this.data.yaopinleixing + '%'
}
...
res = await page(`yaopinleixing`, searchForm);
} else {
res = await list(`yaopinleixing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/yaopinleixing/update-and-add`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yaopinleixing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/yaopinleixing/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/yaopinleixing/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
let isAuthObj={}
if(this.data.isAuthStatus){
//111 yaopinleixing Yaopinleixing yaopinleixing ${pathname}
console.log('Yaopinleixing  yaopinleixing')
isAuthObj['addAuth']   = utils.isAuth("yaopinleixing", "新增")
isAuthObj['delAuth']  = utils.isAuth("yaopinleixing", "删除")
isAuthObj['editAuth']   =utils.isAuth("yaopinleixing", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("yaopinleixing", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("yaopinleixing", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("yaopinleixing", "修改")
}
//前后台权限判断
...
let user = wx.getStorageSync("userid")?wx.getStorageSync('userSession'):{};
    if(this.data.isAuthStatus){
        ...
        resList = await page("yaopinleixing",obj)
    }else{
         resList= await list("yaopinleixing",obj)
    }
