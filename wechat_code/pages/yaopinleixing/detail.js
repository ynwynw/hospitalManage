...
if (id) {
const {data} = await detail("yaopinleixing",id)
this.setData({
payAuth:utils.isAuthFront('yaopinleixing','支付')
})
...
const resUpdate = await update('yaopinleixing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','yaopinleixing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yaopinleixing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','yaopinleixing');
